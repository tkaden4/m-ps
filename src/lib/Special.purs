...
fmApply env closure Nil Nil tree =
    asExpr (eval (Tuple closure tree)) >>= curry eval env
...
def' env (names : value : Nil) = case names of
    SymbolTree name ->
        pure $ Define $ Env $ Map.singleton name $ eval (Tuple env value)
...
int' = Define $ Env $ Map.fromFoldable
  ...
  where
    ...
    mul' env (a : b : Nil) = do
        ...
        evB <- asInteger b
        pure $ IntValue $ evA * evB
    div' :: Partial => Env -> List (EvalResult Value) -> EvalResult Value
    div' env (a : b : zero : Nil) = do
        evA <- asInteger a
        evB <- asInteger b
        if evB == 0
        then zero
        else pure $ IntValue $ (evA / evB)
