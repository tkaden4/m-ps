...
evalBlock' env found errors defer Nil = if found
    then evalBlock' env false Set.empty Nil defer
...
apply env (Macro n f) args =
    ...
    in  if argsLength < n
            then pure $ Macro (n - argsLength) $ \env args' ->
...
apply env (Define defs) args = apply env (Macro 1 $ \env exprs -> 
    case exprs of
...
applyFn env (Function n f) args =
    let argsLength = length args
    in  if argsLength < n
            then pure $ Function (n - argsLength) $ \env args' ->
